...
t_ic = torch.tensor(0.0, requires_grad=True).view(-1, 1).to(device)
u_ic = torch.tensor(u0).view(-1, 1).to(device)
v_ic = torch.tensor(v0).view(-1, 1).to(device)
...
collocation_points_np = collocation_points.detach().cpu().numpy()
...
def save_frame(epoch):
    ...
    plt.plot(t_test.detach().cpu().numpy(), pinn(t_test).detach().cpu().numpy(),
...
plt.figure(figsize=(10, 6))
plt.plot(t_test.detach().cpu().numpy(), u_pred_final.detach().cpu().numpy(), label='PINN Prediction', color='red', linewidth=2)
plt.scatter(collocation_points_np, pinn(collocation_points).detach().cpu().numpy(), s=10, label='Collocation Points',
            color='red', alpha=0.5)
