# PyTorch training parameters of white-box ODE for a damped pendulum with a PyTorch RK4 integrator
import math
import torch
...
# -----------------------------
# We'll learn physical parameters beta and ell (both > 0).
# Use softplus to keep them positive; initialize near truth.

class PendulumODE(nn.Module):
    def __init__(self, beta_init=0.1, ell_init=1.2, g=9.81):
        super().__init__()
        # Unconstrained parameters -> map to positive via softplus
        self._beta = nn.Parameter(torch.tensor([math.log(math.e**beta_init - 1.0)], device=device))
        self._ell  = nn.Parameter(torch.tensor([math.log(math.e**ell_init  - 1.0)], device=device))
        self.softplus = nn.Softplus()
        self.g = g

    @property
    def beta(self):
        return self.softplus(self._beta)

    @property
    def ell(self):
        return self.softplus(self._ell)

    def f(self, x):
        return pendulum_rhs(x, self.beta, self.ell, self.g)
...
for epoch in range(501):
    ...
    if epoch % 100 == 0:
        print(f"[{epoch:04d}] loss={loss.item():.6f}  beta={model.beta.item():.4f}  ell={model.ell.item():.4f}")
...
with torch.no_grad():
    ...
    print(f"Estimated beta={model.beta.item():.4f} (true {beta_true})")
    print(f"Estimated ell ={model.ell.item():.4f} (true {ell_true})")
